...
# for each logline, extract *.go:Lx and figure out its function
	...
	[ $? -ne 0 ] && echo $line >> $LOGFILE.anot && continue
	LINENUM=$(echo $line | grep -o -E '\.go:[0-9]+' | cut --complement -c 1-4)
	find_GOFILE_path
	[ -z $GOFILE_PATH ] && echo "Path for $GOFILE not found" && echo $line >> $LOGFILE.anot && continue
	GO_PKG=$(head -n 50 $GOFILE_PATH | grep 'package' | cut -c 9-)
	echo "$line ++++++++ package $GO_PKG: $GOFILE_PATH: $(head -n $LINENUM $GOFILE_PATH | grep -E '^func ' | tail -n 1)" >> $LOGFILE.anot
done < $LOGFILE
